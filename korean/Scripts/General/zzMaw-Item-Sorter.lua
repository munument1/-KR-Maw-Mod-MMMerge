...
function events.KeyDown(t)
    if Game.CurrentScreen == 7 and Game.CurrentCharScreen == 103 then
        if t.Key == currentBagSortKey then
            sortInventory(false)
            Game.ShowStatusText("인벤토리를 정렬했습니다")
        elseif t.Key == partyBagSortKey then
            sortInventory(true)
            Game.ShowStatusText("모든 인벤토리를 정렬했습니다")
        elseif t.Key == AlchemyBagKey then
            ...
            if vars.alchemyPlayer == Game.CurrentPlayer then
                ...
                Game.ShowStatusText("정렬 시 연금술 우선 대상 없음")
            else
                vars.alchemyPlayer = Game.CurrentPlayer
                Game.ShowStatusText(Party[Game.CurrentPlayer].Name .. " → 정렬 시 연금술 아이템 담당")
            end
        end
    end
end
...
function events.KeyDown(t)
    if Game.CurrentScreen == 7 and Game.CurrentCharScreen == 103 then
        if t.Key == multiBagSortKey then
            ...
            Game.ShowStatusText("정렬 완료: " .. name .. "의 모든 가방")
		end
	end
end
    ...
    if Game.CurrentScreen == 7 and Game.CurrentCharScreen == 103 then
        if t.Key == partyMultyBagSortKey then
			...
			if Party.High==0 then
				...
				gigaChadSort()
			    Game.ShowStatusText("파티의 모든 가방을 정렬했습니다")
			end
		end
	end
end
...
alchemyItemsOrder = {
	...
	1069, 1068, 1064, 1067, 1065, 1063, 1066, 1061, 1062, 1060, 1059, 1058, 1057, 1056, 1055, 1054, 1053, 1052, 1051,
