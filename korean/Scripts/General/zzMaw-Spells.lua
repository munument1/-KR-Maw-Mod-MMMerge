...
function events.KeyDown(t)
	if Game.CurrentScreen==8 and t.Key==removeBuffsKey then
		vars.mawbuff={}
		Game.ShowStatusText("모든 버프가 제거되었습니다")
	end
end
...
function events.PlayerCastSpell(t)
	...
	if t.SpellId==49 then
		if not t.RemoteData then
			...
			if gotCrit then
				Game.ShowStatusText(string.format("회복량: " .. round(totHeal) .. " 생명력 (치명타)"))
			else
				Game.ShowStatusText(string.format("회복량: " .. round(totHeal) .. " 생명력"))
			end
		end
		...
		elseif t.TargetKind == 4 and t.RemoteData then
			...
			if Party.High==0 and healData[2] then
				Game.ShowStatusText(string.format(name .. ": 회복량 " .. healData[1] .. " 생명력 (치명타)"))
			elseif Party.High==0 then
				Game.ShowStatusText(string.format(name .. ": 회복량 " .. healData[1] .. " 생명력"))
			elseif	healData[2] then
				Game.ShowStatusText(string.format(name .. " → " .. Party[t.TargetId].Name .. ": 회복량 " .. healData[1] .. " 생명력 (치명타)"))
			else
				Game.ShowStatusText(string.format(name .. " → " .. Party[t.TargetId].Name .. ": 회복량 " .. healData[1] .. " 생명력"))
			end
	...
	if t.SpellId == 68 then
		...
		if not t.RemoteData then
			...
			if gotCrit then
				Game.ShowStatusText(string.format("회복량: " .. round(tooltipHeal) .. " 생명력 (치명타)"))
			else
				Game.ShowStatusText(string.format("회복량: " .. round(tooltipHeal) .. " 생명력"))
			end
		end
		--end of healing calculation
		...
		elseif t.TargetKind == 4 and t.RemoteData then
			...
			if Party.High==0 and healData[2] then
				Game.ShowStatusText(string.format(name .. ": 회복량 " .. healData[3] .. " 생명력 (치명타)"))
			elseif Party.High==0 then
				Game.ShowStatusText(string.format(name .. ": 회복량 " .. healData[3] .. " 생명력"))
			elseif	healData[2] then
				Game.ShowStatusText(string.format(name .. " → " .. Party[t.TargetId].Name .. ": 회복량 " .. healData[3] .. " 생명력 (치명타)"))
			else
				Game.ShowStatusText(string.format(name .. " → " .. Party[t.TargetId].Name .. ": 회복량 " .. healData[3] .. " 생명력"))
			end
		elseif t.TargetKind == 4 and not t.RemoteData then
			local maxHP = GetMaxHP(Party[t.TargetId])
			local overheal = math.max(0, round(totHeal) - (maxHP - Party[t.TargetId].HP))
...
if t.SpellId == 77 then
  ...
  if not t.RemoteData then
    ...
    if gotCrit then
      Game.ShowStatusText("파티 회복량: " .. tooltipHeal .. " 생명력 (치명타)")
    else
      Game.ShowStatusText("파티 회복량: " .. tooltipHeal .. " 생명력")
    end
  end
  ...
  if not t.RemoteData then
    ...
    if crit then
      ...
      Game.ShowStatusText(name .. ": 파티 회복량 " .. shown .. " 생명력")
    end
  end
end
...
function doSharedLife(amount, spellQueueData)
	...
	if gotCrit then
		Game.ShowStatusText(string.format("생명 공유 회복량: " .. round(totHeal) .. " 생명력 (치명타)"))
	else
		Game.ShowStatusText(string.format("생명 공유 회복량: " .. round(totHeal) .. " 생명력"))
	end
...
function events.LoadMap()
	if table.find(flyAllowedMaps,Map.Name) then 
		if Game.BolsterAmount>100 then
			Sleep(5)
			Game.ShowStatusText("이곳에서는 제한 없이 비행할 수 있습니다")
		end
	end
end
