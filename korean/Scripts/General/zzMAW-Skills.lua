			...
			if vars.chargeCooldown==0 then
				if Mouse:GetTarget().Kind==3 then
					...
					if dist<3000 and dist>500 then
						...
						Game.ShowStatusText(string.format("%s의 돌진이 적을 기절시킵니다",Party[0].Name))
						...
						Game.ShowStatusText("사정거리 밖입니다")
					end
				end
			else
				Game.ShowStatusText(string.format("돌진 재사용 대기시간이 %s초 남았습니다",vars.chargeCooldown))
			end
		end
	end
end
	...
	if t.Key == healthPotionKey then
		if vars.healthPotionCooldown==0 then
			...
			if heal then
				...
				Game.ShowStatusText(string.format("생명력 물약으로 생명력 %s 회복",healAmount))
			end
		else
			Game.ShowStatusText(string.format("생명력 물약 재사용 대기시간이 %s초 남았습니다",vars.healthPotionCooldown))
		end
	end
end
	...
	if t.Key == manaPotionKey then
		if vars.manaPotionCooldown==0 then
			...
			if heal then
				...
				Game.ShowStatusText(string.format("마나 물약으로 마나 %s 회복",spAmount))
			end
		else
			Game.ShowStatusText(string.format("마나 물약 재사용 대기시간이 %s초 남았습니다",vars.manaPotionCooldown))
		end
	end
end
	...
	if t.Key==const.Keys.P then
		if Game.CurrentCharScreen==101 and Game.CurrentScreen==7 then
			...
			if vars.covering[Game.CurrentPlayer] then
				...
				Game.ShowStatusText("엄호 비활성화")
			else
				vars.covering[Game.CurrentPlayer]=true
				Game.ShowStatusText("엄호 활성화")
			end
		end
	end
	if t.Key==const.Keys.M then
		if Game.CurrentCharScreen==101 and Game.CurrentScreen==7 then
			...
			if vars.manaShield[Game.CurrentPlayer] then
				...
				Game.ShowStatusText("마나 방패 비활성화")
			else
				vars.manaShield[Game.CurrentPlayer]=true
				Game.ShowStatusText("마나 방패 활성화")
			end
		end
	end
	...
	if t.Action==121 then
		if t.Param==50 then
			...
			if s==29 and pl.SkillPoints>29 then
				...
				if s==0 then
					Skillz.set(pl,53,1)
					Game.ShowStatusText("반격을 습득했습니다!!")
				end
			end
			if s==30 then 
				t.Handled=true
