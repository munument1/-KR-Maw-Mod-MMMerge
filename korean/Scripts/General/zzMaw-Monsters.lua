...
function events.CanCastLloyd(t)
	...
	if Party.EnemyDetectorYellow or Party.EnemyDetectorRed then
		t.Result=false
		Sleep(1)
		Game.ShowStatusText("지금은 순간이동할 수 없습니다")
	end
end
	...
	if vars.madnessMode and showDeathCounter then
		...
		if t.Action==125 or t.Action==132 then
			if vars.lastHitTime and vars.lastHitTime~=0 and loadWarning then
				t.Handled=true
				Game.ShowStatusText("지금 불러오면 사망으로 처리됩니다. 계속하시겠습니까?")
			end
			loadWarning=false
		else
			loadWarning=true
	...
	if (Map.IndoorOrOutdoor==1 and mapvars.monsterMap and mapvars.completed==nil) or (Map.IndoorOrOutdoor==2 and mapvars.completed==nil) then
		...
		if m/n>=requiredRateo then
			...
			if vars.dungeonCompletedList[name] and not vars.madnessMode then
				...
				if Game.CurrentScreen~=22 then
					if vars.insanityMode then
						if disableCompletitionMessage then
							Game.ShowStatusText("던전 완료!")
						else
							Game.EscMessage(string.format("던전 완료!"))
						end
					else
						if disableCompletitionMessage then
							Game.ShowStatusText("던전 완료!\n이제 다시 초기화할 수 있습니다.")
				...
				if Game.CurrentScreen~=22 then
					if disableCompletitionMessage then
						Game.ShowStatusText("맵 완료! 획득: " .. experience .. " 경험치, " .. gold .. " 금화 및 제작 재료")
					else
						Game.EscMessage(string.format("맵 완료! 획득: " .. experience .. " 경험치, " .. gold .. " 금화 및 제작 재료"))
					end
				end
				return
			end
		...
		if mapvars.monsterMap and mapvars.monsterMap.cleared==false and m/n>=0.65 and Game.BolsterAmount>=300 then
			mapvars.monsterMap.cleared=true
		 	if Game.CurrentScreen~=22 then
		 		if disableCompletitionMessage then
		 			Game.ShowStatusText("몬스터들이 약화되어 더 이상 부활할 수 없습니다")
		 		else
		 			Game.EscMessage(string.format("몬스터들이 약화되어 더 이상 부활할 수 없습니다"))
		 		end
		 	end
		end
	end
		...
		if table.find(transformedList, obj.Type) and (dist<128 or (obj.X==lastLocation[i][1] and obj.Y==lastLocation[i][2])) and obj.Spell==0 then
			...
			if triggeredByPlayer then
				...
				if covered then
					...
					Game.ShowStatusText(Party[coverPlayerIndex].Name .. " → 엄호: " .. Party[target].Name)
...
function events.KeyDown(t)
	if t.Key==teleportKey then
		...
		if objCount == 0 then
			Game.ShowStatusText("순간이동: " .. count .. "마리의 몬스터")
		elseif count == 0 then
			Game.ShowStatusText("순간이동: " .. objCount .. "개의 제작 재료")
		else
			Game.ShowStatusText("순간이동: " .. count .. "마리의 몬스터와 " .. objCount .. "개의 제작 재료")
		end
	end
end
