#!/usr/bin/env python3
"""Localize only player-facing POTION/POTNOTES display fields in a zMaw stage.

Columns after ID/Name/Description/Effect are recipe/control matrix data.  This
script patches only columns 1..3 (zero-based fields 1..3), verifies every
display field has a Korean result, and verifies every later matrix field is
unchanged.  POTION.TXT and POTNOTES.TXT are translated independently because
MAW intentionally gives many rows different names/descriptions between them.
"""
...
DESCRIPTION_MAP = {
    ...
    "Red Potion": "붉은색 물약",
    "Blue Potion": "푸른색 물약",
    "Yellow Potion": "노란색 물약",
    "Orange Potion": "주황색 물약",
    "Purple Potion": "보라색 물약",
    "Green Potion": "녹색 물약",
    "White Potion": "흰색 물약",
    "Black Potion": "검은색 물약",
...
STAT_KO = {
    ...
    "Endurance": "지구력", "Accuracy": "정확도", "Speed": "속도",
    "Luck": "행운",
}
RESIST_KO = {
...
SPELL_KO = {
    "Haste": "가속", "Heroism": "영웅심", "Bless": "축복",
    "Preservation": "보존", "Shield": "방패", "Protection": "보호",
    "Stoneskin": "돌가죽",
}
WEAPON_EFFECT_KO = {
    "flame": "화염", "frost": "냉기", "poison": "독",
    "sparks": "전격", "swiftness": "신속",
}
COLOR_KO = {
    "Red": "붉은색", "Blue": "푸른색", "Yellow": "노란색", "Gray": "회색",
}

EXACT_EFFECT = {
    "None": "없음",
    "Boost Potion": "물약 강화",
    "Heal 10+skill HP": "생명력 10 + 연금술 기술 수치 회복",
    "Restore 10+skill MP": "주문력 10 + 연금술 기술 수치 회복",
    ...
    "Add strength to toughness of item": "아이템 견고함에 물약 위력 추가",
    ...
    "Set Age Temp to 0": "마법적 노화 수치를 0으로 설정",
...
def translate_name(text: str) -> str | None:
    return NAME_MAP.get(norm_key(text))


def translate_description(text: str) -> str | None:
    return DESCRIPTION_MAP.get(norm_key(text))
...
def localize_table(stage_path: Path, audit: dict[int, dict[str, str]], prefix: str):
    doc = TsvDocument(read_source_text(stage_path))
    ...
    for record_id, audit_row in audit.items():
        ...
        if row is None:
            errors.append({"table": stage_path.name, "id": record_id, "type": "missing_row"})
            continue
        if len(row.fields) < 4:
            errors.append({"table": stage_path.name, "id": record_id, "type": "short_row"})
            continue

        original_tail = list(row.fields[4:])
        source_values = tuple(decode_field(row.fields[i]) for i in (1, 2, 3))
        guard_values = tuple(audit_row[f"{prefix}_{field}"] for field in ("name", "description", "effect"))
        if source_values != guard_values:
            errors.append({
                "table": stage_path.name,
                "id": record_id,
                "type": "audit_source_mismatch",
                "stage": source_values,
                "audit": guard_values,
            })
            continue
        ...
        translated = (
            translate_name(source_values[0]),
            translate_description(source_values[1]),
            translate_effect(source_values[2]),
        )
        for field_name, source, value in zip(("name", "description", "effect"), source_values, translated):
            if value is None:
                errors.append({
                    "table": stage_path.name,
                    "id": record_id,
                    "type": "untranslated_display_field",
                    "field": field_name,
                    "source": source,
                })
        ...
        for index, value in enumerate(translated, 1):
            ...
            row.fields[index] = encode_field(value)
            changed += 1

        if row.fields[4:] != original_tail:
            errors.append({"table": stage_path.name, "id": record_id, "type": "matrix_changed"})

        review.append({
            "table": stage_path.name,
            "id": str(record_id),
            "source_name": source_values[0], "ko_name": translated[0] or "",
            "source_description": source_values[1], "ko_description": translated[1] or "",
            "source_effect": source_values[2], "ko_effect": translated[2] or "",
        })
...
def main() -> int:
    ...
    for filename, prefix in (("POTION.TXT", "potion"), ("POTNOTES.TXT", "potnotes")):
        ...
        doc, changed, errors, review = localize_table(path, audit, prefix)
        replacements[filename] = changed
    ...
    fields = [
        "table", "id", "source_name", "ko_name", "source_description",
        "ko_description", "source_effect", "ko_effect",
    ]
    ...
    print(json.dumps({
        ...
        "total_localized_fields": report["total_localized_fields"],
        "errors": all_errors,
