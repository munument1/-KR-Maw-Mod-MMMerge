#!/usr/bin/env python3
"""Promote proven MAW 4.5 monster information-box labels.

``effectNames`` is appended directly to ``t.EffectsHeader.Text`` and the local
``debuffNames`` table is rendered under the same information box together with
remaining debuff duration. Only literals in these two named tables are promoted.
"""
...
TABLE_RE = re.compile(r"^(?:local\s+)?(effectNames|debuffNames)\s*=\s*\{")
...
def main() -> int:
    ...
    for row in rows:
        ...
        if key in safe:
            row["patchable"] = "yes"
            row["reason"] = "proven_monster_info_label"
            promoted += 1

    write_rows(occ_path, rows)
    report = json.loads(report_path.read_text(encoding="utf-8")) if report_path.exists() else {}
    report["patchability_reasons"] = dict(sorted(Counter(r.get("reason", "") for r in rows).items()))
    report["proven_monster_info_lines"] = len(safe)
    report["proven_monster_info_promotions"] = promoted
